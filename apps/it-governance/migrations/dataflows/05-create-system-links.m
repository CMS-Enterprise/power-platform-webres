section Section1;
shared cr69a_systemintakestaginglinkedsystems = let
    ...
    MapMultiSelectToDataverseString = (raw as any, map as record) as nullable text =>
        let
            ...
            ints   = List.Transform(
                        keys,
                        each if _ <> null and Record.HasFields(map, _) then Record.Field(map, _) else null
                    ),
            good   = List.RemoveNulls(ints),
            result = if List.Count(good) = 0
                     then null
                     else Text.Combine(List.Transform(good, each Text.From(_)), ",")
        in
            result,
