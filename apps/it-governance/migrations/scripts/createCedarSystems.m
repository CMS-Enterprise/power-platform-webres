let
    ...
    #"Added ATO Effective Date" = Table.AddColumn(
        ...
        each
            let
                d = try Date.FromText([new_atoeffectivedate]) otherwise null
            in
                if d <> null and d >= #date(1753, 1, 1) then
                    d
                else
                    null,
        type date
    ...
    #"Added ATO Expiration Date" = Table.AddColumn(
        ...
        each
            let
                d = try Date.FromText([new_atoexpirationdate]) otherwise null
            in
                if d <> null and d >= #date(1753, 1, 1) then
                    d
                else
    ...
    #"Added Cedar State" = Table.AddColumn(
        ...
        each
            let
                state = Text.Upper(Text.Trim(Text.From([new_state])))
            in
