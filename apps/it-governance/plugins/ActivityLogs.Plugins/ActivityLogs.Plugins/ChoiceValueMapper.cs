...
namespace SystemIntake.Plugins
{
    internal static class ChoiceValueMapper
    {
        ...
        internal static string GetMappedChoiceLabel(
            IOrganizationService service,
            string targetEntityName,
            string targetFieldName,
            object mappedValue)
        {
            var mappedChoice = mappedValue as OptionSetValue;
            if (mappedChoice == null)
                return null;

            var targetMetadata = RetrievePicklistMetadata(service, targetEntityName, targetFieldName);
            return targetMetadata == null ? null : GetOptionLabel(targetMetadata, mappedChoice.Value);
        }
